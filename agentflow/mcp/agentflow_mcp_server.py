...
from agentflow.core.project_scanner import ProjectScanner

# A2A imports - optional
try:
...
class AgentFlowMCPServer:
    def __init__(self):
        """Initialize AgentFlow MCP server with lazy loading."""
        # Lazy initialization - only create when first used
        self._smart_orchestrator = None
        self._delegation_pool = None
        self.project_scanner = None
        self.project_index = None

        # Optional A2A integration - also lazy
    ...
    def smart_orchestrator(self):
        """Lazy load SmartOrchestrator."""
        if self._smart_orchestrator is None:
            self._smart_orchestrator = SmartOrchestrator()
        return self._smart_orchestrator

    @property
    def delegation_pool(self):
        """Lazy load DelegationPool."""
        if self._delegation_pool is None:
            self._delegation_pool = DelegationPool()
        return self._delegation_pool

    @property
    def a2a_adapter(self):
        """Lazy load A2A adapter."""
        if self._a2a_adapter is None and A2A_AVAILABLE:
            try:
                self._a2a_adapter = AgentFlowA2AAdapter(self)
                self._a2a_adapter.register()
                self.a2a_enabled = True
                logger.info(
                    "agentflow_mcp_server_a2a_initialized",
                    a2a_enabled=True,
                    a2a_agent_id="agentflow",
                    capabilities=["TASK_DELEGATION", "PARALLEL_EXECUTION", "VALIDATION"]
                )
            except Exception as e:
                logger.warning("a2a_registration_failed", error=str(e))
    ...
    async def _delegate_task(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Use SmartOrchestrator.orchestrate method
        result = self.smart_orchestrator.orchestrate(
            task=task_description,
            capabilities=[],  # Let orchestrator infer
            custom_instructions=str(context) if context else None
        )
        ...
        return {
            ...
            "model": self.smart_orchestrator.current_provider,
            "status": "completed" if result.success else "failed",
            "result": result.final_output if result.success else None,
...
def main():
    if len(sys.argv) > 1 and sys.argv[1] == 'test':
        ...
        logging.getLogger('agentflow').setLevel(logging.ERROR)
